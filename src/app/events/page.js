export default function Events() {
  const dataDummy = {
    ...
    data: [
      {
        events: {
          id: "ds_zXj9fyBJnCNSx0R",
          title: "Kampanye Tandingan",
          description: "KAMPANYE TANDINGAN, live political comedy show yang berkolaborasi dengan Musuh Masyarakat (Coki Pardede, Tretan Muslim, dan Adriano Qalbi) dan menampilkan line-up keren lainnya!",
          image: "https://res.cloudinary.com/dgt0nrylf/image/upload/v1707373151/eventsmaker/ybg4u9js4k1khr7t7j5b.png",
          dateTime: "9 Februari 2024",
      ...
      {data.map((d, idx) => (
        <div key={idx} className="flex flex-col gap-y-4">
          <div className="flex flex-col gap-y-2">
            <h2 className="text-2xl">{d.events.title}</h2>
            <p>{d.events.description}</p>
            <p>{d.events.dateTime}</p>
          </div>
          <div className="flex flex-col gap-x-2">
            {d.participants.length > 0
              ? d.participants.map((participant, idx) => (
